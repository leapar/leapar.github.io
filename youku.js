/**
 * Created by wangxh on 2016/6/16.
 */
...
function getCookiePath(videoId) {
	$.ajax({
		url: "http://v.youku.com/v_show/id_" + videoId + ".html",
		headers: {
...
function getFlvPath(url, videoId, callback) {
	$.ajax({
		url: url,
		headers: {
			"Referer": "v.youku.com/v_show/id_" + videoId + ".html" // 桳嵄??婍晄堯?廋夵??媮?
...
function parseYoukuCode_new(url, callback) {
...
function parseYoukuCode(url, callback) {
...
function PlayListData(a, b, c) {
	...
	for (var g = 0; g < e.length; g++) {
		for (var k = e[g], l = {}, m = {}, n = [], j = 0; j < b.length; j++) {
			var o = b[j];
			if (k == o.audio_lang) {
				if (!d.isValidType(o.stream_type)) continue;
				...
				for (var v = 0; v < t.length; v++) {
					...
					if (null == w) break;
					var x = {};
					x.no = v, x.size = w.size, x.seconds = Number(w.total_milliseconds_video) / 1e3, x.milliseconds_video = Number(o.milliseconds_video) / 1e3, x.key = w.key, x.fileId = this.getFileId(o.stream_fileid, v), x.src = this.getVideoSrc(j, v, a, o.stream_type, x.fileId), x.type = p, u.push(x)
				}
				l[p] = u
			}
		}
